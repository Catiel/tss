...
public class InventorySystemSimulatorAleatorio extends JFrame {
    ...
    private JButton btnOptimizar, btnActualizar, btnGenerarAleatorio, btnGraficas;
    ...
    public InventorySystemSimulatorAleatorio() {
    ...
    private JPanel crearPanelControl() {
        ...
        btnGenerarAleatorio = crearBoton("Generar Aleatorio", new Color(255, 165, 0), 170, 40);
        btnActualizar = crearBoton("Actualizar Tabla", new Color(237, 125, 49), 170, 40);
        btnOptimizar = crearBoton("Ejecutar Optimización (OptQuest)",
                                  new Color(68, 114, 196), 280, 45);
        btnGraficas = crearBoton("Ver Gráficas", new Color(112, 173, 71), 150, 40);
    ...
    private JButton crearBoton(String texto, Color bg, int ancho, int alto) {
        ...
        btn.setFont(new Font("Calibri", Font.BOLD, texto.length() > 20 ? 13 : 14));
    ...
    private void generarDatosAleatorios() {
        ...
        // Generar Cantidad de pedido (entre 200 y 400, múltiplo de 5)
        int cantidadPedido = Config.ORDEN_MIN + (random.nextInt((Config.ORDEN_MAX - Config.ORDEN_MIN) / Config.ORDEN_PASO + 1) * Config.ORDEN_PASO);
        txtCantidadPedido.setText(String.valueOf(cantidadPedido));

        // Generar Punto de reorden (entre 200 y 400, múltiplo de 10)
        int puntoReorden = Config.REORDEN_MIN + (random.nextInt((Config.REORDEN_MAX - Config.REORDEN_MIN) / Config.REORDEN_PASO + 1) * Config.REORDEN_PASO);
        txtPuntoReorden.setText(String.valueOf(puntoReorden));

        // Generar Costo del pedido (entre 20 y 100, ENTERO)
        int costoPedido = 20 + random.nextInt(81); // 20 a 100
        txtCostoPedido.setText(String.valueOf(costoPedido));

        // Generar Costo de tenencia (entre 0.10 y 0.50, con 2 decimales)
        double costoTenencia = 0.10 + (random.nextDouble() * 0.40);
        txtCostoTenencia.setText(String.format("%.2f", costoTenencia));

        // Generar Costo de ventas perdidas (entre 50 y 200, ENTERO)
        int costoVentasPerdidas = 50 + random.nextInt(151); // 50 a 200
        txtCostoVentasPerdidas.setText(String.valueOf(costoVentasPerdidas));

        // Actualizar inventario inicial (igual a cantidad de pedido)
        txtInventarioInicial.setText(String.valueOf(cantidadPedido));

        // Actualizar las variables de configuración
        Config.COSTO_PEDIDO = costoPedido;
        Config.COSTO_TENENCIA = costoTenencia;
        Config.COSTO_VENTAS_PERDIDAS = costoVentasPerdidas;

        // Actualizar las variables actuales
        cantidadPedidoActual = cantidadPedido;
        puntoReordenActual = puntoReorden;

        // Simular automáticamente con los nuevos valores
        simularYMostrarEnTabla(cantidadPedido, puntoReorden);

        // Mostrar mensaje de confirmación
        JOptionPane.showMessageDialog(this,
            String.format("Datos aleatorios generados:\n\n" +
                         "Cantidad de pedido: %d\n" +
                         "Punto de reorden: %d\n" +
                         "Costo del pedido: $%d\n" +
                         "Costo de tenencia: $%.2f\n" +
                         "Costo de ventas perdidas: $%d",
                         cantidadPedido, puntoReorden, costoPedido, costoTenencia, costoVentasPerdidas),
            "Datos Generados",
    ...
    private void actualizarTablaConValoresActuales() {
        try {
            ...
            String cantPedidoStr = txtCantidadPedido.getText().trim();
            String puntoReordenStr = txtPuntoReorden.getText().trim();
            String costoPedidoStr = txtCostoPedido.getText().trim();
            String costoTenenciaStr = txtCostoTenencia.getText().trim().replace(",", ".");
            String costoVentasStr = txtCostoVentasPerdidas.getText().trim();

            // Validar que no estén vacíos
            if (cantPedidoStr.isEmpty() || puntoReordenStr.isEmpty() ||
                costoPedidoStr.isEmpty() || costoTenenciaStr.isEmpty() || costoVentasStr.isEmpty()) {
                JOptionPane.showMessageDialog(this,
            ...
            // Parsear valores - Costo pedido y ventas perdidas son ENTEROS
            cantidadPedidoActual = Integer.parseInt(cantPedidoStr);
            puntoReordenActual = Integer.parseInt(puntoReordenStr);
            Config.COSTO_PEDIDO = Double.parseDouble(costoPedidoStr);
            Config.COSTO_TENENCIA = Double.parseDouble(costoTenenciaStr);
            Config.COSTO_VENTAS_PERDIDAS = Double.parseDouble(costoVentasStr);

            // Validar rangos
            if (cantidadPedidoActual < 0 || puntoReordenActual < 0) {
                JOptionPane.showMessageDialog(this,
                    "La cantidad de pedido y punto de reorden deben ser positivos",
                    "Valores Inválidos",
                    JOptionPane.WARNING_MESSAGE);
                return;
            }

            if (Config.COSTO_PEDIDO < 0 || Config.COSTO_TENENCIA < 0 || Config.COSTO_VENTAS_PERDIDAS < 0) {
                JOptionPane.showMessageDialog(this,
            ...
            JOptionPane.showMessageDialog(this,
                ...
                "- Costo de ventas perdidas: 100 (entero)",
                "Error de Formato",
    ...
    private void ejecutarOptimizacion() {
        ...
        try {
            ...
            // Validar que no estén vacíos
            if (costoPedidoStr.isEmpty() || costoTenenciaStr.isEmpty() || costoVentasStr.isEmpty()) {
                JOptionPane.showMessageDialog(this,
                    "Por favor complete todos los campos de costos antes de optimizar",
                    "Campos Vacíos",
                    JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            JOptionPane.showMessageDialog(this,
                "Por favor ingrese valores numéricos válidos en los costos.\n\n" +
                "Formatos correctos:\n" +
                "- Costo del pedido: 50 (entero)\n" +
                "- Costo de tenencia: 0.20 o 0,20 (decimal)\n" +
                "- Costo de ventas perdidas: 100 (entero)",
    ...
    private void mostrarHistograma() {
        ...
        if (datos.length == 0) {
            JOptionPane.showMessageDialog(this, "No hay datos para mostrar. Ejecute primero la optimización.");
            return;
        }
        ...
        rangeAxis.setLabel("Frecuencia");
        rangeAxis.setTickLabelFont(new Font("Arial", Font.PLAIN, 10));
        rangeAxis.setStandardTickUnits(NumberAxis.createIntegerTickUnits());

        NumberAxis rangeAxis2 = new NumberAxis("Probabilidad");
        rangeAxis2.setTickLabelFont(new Font("Arial", Font.PLAIN, 10));
        double maxFreq = rangeAxis.getUpperBound();
        rangeAxis2.setRange(0, maxFreq / datos.length);
        ...
        marker.setLabelFont(new Font("Arial", Font.BOLD, 11));
        ...
        JLabel lblStats = new JLabel(String.format("%,d pruebas                Vista de frecuencia                %,d mostrados",
            Config.NUM_PRUEBAS_MC, datos.length));
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() ->
            new InventorySystemSimulatorAleatorio().setVisible(true));
    }
}