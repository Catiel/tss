...
public class VisualLocationManager {
    ...
    private final Map<String, BrewerySimulationGUI.Point2D> positions;
    private static final double LOCATION_SIZE = 80;
    
    public VisualLocationManager(Map<String, BrewerySimulationGUI.Point2D> positions) {
        this.positions = positions;
    }
    
    public void render(GraphicsContext gc, SimulationEngine engine) {
        for (Map.Entry<String, BrewerySimulationGUI.Point2D> entry : positions.entrySet()) {
            String locationName = entry.getKey();
            BrewerySimulationGUI.Point2D pos = entry.getValue();
    ...
    private void renderLocation(GraphicsContext gc, String name, BrewerySimulationGUI.Point2D pos, Location location) {
        ...
        gc.setFill(gradient);
        gc.fillRoundRect(pos.x, pos.y, LOCATION_SIZE, LOCATION_SIZE, 12, 12);
        
        // Borde
        gc.setEffect(null);
        gc.setStroke(Color.WHITE);
        gc.setLineWidth(3);
        gc.strokeRoundRect(pos.x, pos.y, LOCATION_SIZE, LOCATION_SIZE, 12, 12);
        ...
        gc.fillText(icon, pos.x + LOCATION_SIZE/2, pos.y + LOCATION_SIZE/2 + 15);
        
        // Dibujar nombre de la locación con fondo semitransparente
        gc.setFill(Color.color(0, 0, 0, 0.7));
        gc.fillRoundRect(pos.x - 5, pos.y - 25, LOCATION_SIZE + 10, 20, 5, 5);
        gc.setFill(Color.web("#ECF0F1"));
        gc.setFont(Font.font("Arial", FontWeight.BOLD, 10));
        gc.setTextAlign(TextAlignment.CENTER);
        String displayName = formatLocationName(name);
        gc.fillText(displayName, pos.x + LOCATION_SIZE/2, pos.y - 10);
        ...
        gc.fillRoundRect(pos.x - 5, pos.y - 28, LOCATION_SIZE + 10, 22, 6, 6);
        gc.setFill(Color.WHITE);
        gc.setFont(Font.font("Arial", FontWeight.BOLD, 11));
        String locName = formatLocationName(name);
        gc.fillText(locName, pos.x + LOCATION_SIZE/2, pos.y - 12);
        
        // Indicador de ocupación ABAJO con fondo oscuro
        gc.setFill(Color.color(0, 0, 0, 0.85));
        gc.fillRoundRect(pos.x + 5, pos.y + LOCATION_SIZE + 8, LOCATION_SIZE - 10, 22, 6, 6);
        gc.setFont(Font.font("Arial", FontWeight.BOLD, 13));
        gc.setFill(Color.YELLOW);
        String occupancyText = currentOccupancy + "/" + (capacity == Integer.MAX_VALUE ? "∞" : capacity);
        gc.fillText(occupancyText, pos.x + LOCATION_SIZE/2, pos.y + LOCATION_SIZE + 24);
        
        // Dibujar cola si existe (badge animado)
        if (queueSize > 0) {
            gc.setFill(new RadialGradient(
                0, 0,
                pos.x + LOCATION_SIZE - 7.5, pos.y - 7.5,
                15,
                false,
                CycleMethod.NO_CYCLE,
            ...
            gc.fillOval(pos.x + LOCATION_SIZE - 20, pos.y - 10, 30, 30);
            gc.setStroke(Color.WHITE);
            gc.setLineWidth(2);
            gc.strokeOval(pos.x + LOCATION_SIZE - 20, pos.y - 10, 30, 30);
            gc.setFill(Color.WHITE);
            gc.setFont(Font.font("Arial", FontWeight.BOLD, 13));
            gc.fillText(String.valueOf(queueSize), pos.x + LOCATION_SIZE - 5, pos.y + 8);
        }
        ...
        if (capacity < Integer.MAX_VALUE) {
            double barWidth = 65;
            double barHeight = 8;
            double barX = pos.x + (LOCATION_SIZE - barWidth) / 2;
            double barY = pos.y + LOCATION_SIZE + 30;
     ...
     */
    private void renderAccumulatedEntitiesCircles(GraphicsContext gc, BrewerySimulationGUI.Point2D pos, 
        ...
        double startY = pos.y + (LOCATION_SIZE - totalHeight) / 2;
        ...
        for (int i = 0; i < occupancy; i++) {
            ...
            double startX = pos.x + (LOCATION_SIZE - totalWidth) / 2;
     ...
     */
    private void renderAccumulatedEntitiesCount(GraphicsContext gc, BrewerySimulationGUI.Point2D pos, 
                                                int occupancy) {
        double centerX = pos.x + LOCATION_SIZE/2;
        double centerY = pos.y + LOCATION_SIZE/2;
    ...
    public BrewerySimulationGUI.Point2D getLocationPosition(String name) {
