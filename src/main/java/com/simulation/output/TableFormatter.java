...
public class TableFormatter { // Clase utilitaria para formatear tablas de estadísticas en texto
    ...
    public static String formatDouble(double value, int decimals) { // Método estático para formatear números decimales
                                                                    // con cantidad específica de decimales
        return String.format("%." + decimals + "f", value); // Retorna el número formateado como string con el número de
                                                            // decimales especificado
    }

    public static String formatEntityTable(java.util.Map<String, com.simulation.entities.EntityStatistics> stats) { // Método
                                                                                                                    // estático
                                                                                                                    // para
                                                                                                                    // formatear
                                                                                                                    // la
                                                                                                                    ...
                                                                                                                    // de
                                                                                                                    // entidades
        StringBuilder sb = new StringBuilder(); // Crea un StringBuilder para construir eficientemente la tabla como
                                                // string

        sb.append("\n=== ENTIDAD RESUMEN ===\n\n"); // Agrega el título de la sección de entidades con líneas en blanco
        sb.append(String.format("%-25s %15s %25s %35s %40s %30s %30s %30s\n", // Agrega la fila de encabezados con
                                                                              // formato de columnas alineadas a la
                                                                              // izquierda y derecha
        ...
        for (com.simulation.entities.EntityStatistics stat : stats.values()) { // Itera sobre todas las estadísticas de
                                                                               // entidades en el mapa
            sb.append(String.format("%-25s %15d %25d %35s %40s %30s %30s %30s\n", // Agrega una fila de datos con
                                                                                  // formato de columnas
                    ...
                    formatDouble(stat.getAverageSystemTime(), 2), // Tiempo promedio en sistema formateado con 2
                                                                  // decimales
                    formatDouble(stat.getAverageNonValueAddedTime(), 2), // Tiempo promedio sin valor agregado
                                                                         // formateado con 2 decimales
                    formatDouble(stat.getAverageWaitTime(), 2), // Tiempo promedio de espera formateado con 2 decimales
                    formatDouble(stat.getAverageValueAddedTime(), 2), // Tiempo promedio de operación formateado con 2
                                                                      // decimales
    ...
    public static String formatLocationTable(java.util.Map<String, com.simulation.locations.LocationStatistics> stats) { // Método
                                                                                                                         // estático
                                                                                                                         // para
                                                                                                                         // formatear
                                                                                                                         // la
        ...
        sb.append("\n=== LOCACIÓN RESUMEN ===\n\n"); // Agrega el título de la sección de ubicaciones con líneas en
                                                     // blanco
        sb.append(String.format("%-25s %20s %15s %20s %30s %25s %20s %20s %20s\n", // Agrega la fila de encabezados con
                ...
                "Tiempo Por entrada Promedio (Min)", "Contenido Promedio", "Contenido Máximo", // Siguientes tres
                                                                                               // encabezados
        ...
        for (com.simulation.locations.LocationStatistics stat : stats.values()) { // Itera sobre todas las estadísticas
                                                                                  // de ubicaciones en el mapa
            sb.append(String.format("%-25s %20s %15d %20d %30s %25s %20s %20s %20s\n", // Agrega una fila de datos con
                    ...
                    formatDouble(stat.getScheduledTime() / 60.0, 2), // Tiempo programado convertido de minutos a horas
                                                                     // con 2 decimales
                    stat.getCapacity(), // Capacidad de la ubicación
                    stat.getTotalEntries(), // Total de entradas a la ubicación
                    formatDouble(stat.getAverageTimePerEntry(), 2), // Tiempo promedio por entrada formateado con 2
                                                                    // decimales
                    ...
                    formatDouble(stat.getUtilizationPercent(), 2) // Porcentaje de utilización formateado con 2
                                                                  // decimales
            ));
        }
    ...
    public static String formatResourceTable(java.util.Map<String, com.simulation.resources.ResourceStatistics> stats) {
        StringBuilder sb = new StringBuilder();

        sb.append("\n=== RECURSO RESUMEN ===\n\n");
        sb.append(String.format("%-25s %15s %20s %20s %20s\n",
                "Nombre", "Unidades", "% Utilización", "Tiempo Promedio/Viaje", "Total Viajes"));
        sb.append("-".repeat(110)).append("\n");

        for (com.simulation.resources.ResourceStatistics stat : stats.values()) {
            sb.append(String.format("%-25s %15d %20s %20s %20d\n",
                    stat.getResourceName(),
                    stat.getUnits(),
                    formatDouble(stat.getUtilizationPercent(), 2),
                    formatDouble(stat.getAverageMinutesPerTrip(), 2),
                    stat.getTotalTrips()));
        }
