...
public class OptimizadorSeleccionProyectosManual extends JFrame {
    ...
    private static class Config {
        ...
        static double[] INVERSION = {250000, 650000, 250000, 500000, 700000, 30000, 350000, 70000}; // Ahora es mutable
        static double PRESUPUESTO = 2000000; // Ahora es mutable
    ...
    private JButton btnOptimizar, btnGraficas, btnEditarVariables;
    ...
    public OptimizadorSeleccionProyectosManual() {
    ...
    private void configurarEstiloTabla(JTable tabla) {
        ...
        Color[] colores = {new Color(197, 217, 241), new Color(255, 255, 0), new Color(146, 208, 80), new Color(146, 208, 80), Color.WHITE, Color.WHITE, new Color(0, 176, 240)};
    ...
    private void llenarTablaInicial() {
        ...
        for (int i = 0; i < Config.NUM_PROYECTOS; i++) {
            ...
            modeloTabla.addRow(new Object[]{String.valueOf(i + 1), "1", FMT_MONEY.format(Config.GANANCIA[i]), FMT_PERCENT.format(Config.PROB_EXITO[i]), FMT_MONEY.format(retorno), FMT_MONEY.format(Config.INVERSION[i]), FMT_MONEY.format(0)});
        }
    }
    ...
    private JLabel crearLabelTotal(String titulo, double valor, Color bg) {
        JLabel lbl = new JLabel("<html><center>" + titulo + "<br><b>" + FMT_MONEY.format(valor) + "</b></center></html>");
    ...
    private JPanel crearPanelControl() {
        ...
        btnEditarVariables = crearBoton("Editar Variables", new Color(255, 140, 0), 200, 40);
        btnOptimizar = crearBoton("Ejecutar Optimización (OptQuest)", new Color(68, 114, 196), 320, 45);
        btnGraficas = crearBoton("Ver Gráficas", new Color(112, 173, 71), 180, 40);
        btnGraficas.setEnabled(false);

        btnEditarVariables.addActionListener(e -> mostrarDialogoEdicion());
        btnOptimizar.addActionListener(e -> ejecutarOptimizacion());
        btnGraficas.addActionListener(e -> mostrarHistograma());

        panelBtns.add(btnEditarVariables);
    ...
    private JPanel crearPanelProgreso() {
        ...
        panel.setBackground(Color.WHITE);
        panel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.GRAY), "Panel de control: OptQuest"));
        ...
        progressBar = crearBarraConLabel(Config.NUM_SIMULACIONES, "Simulaciones totales", new Color(0, 32, 96), out -> lblSimulaciones = out);
        progressBarPruebas = crearBarraConLabel(Config.NUM_PRUEBAS_MC, "Pruebas", new Color(0, 176, 80), out -> lblPruebas = out);
    ...
    private JProgressBar crearBarraConLabel(int max, String texto, Color color, java.util.function.Consumer<JLabel> labelOut) {
    ...
    private void mostrarDialogoEdicion() {
        JDialog dialogo = new JDialog(this, "Editar Variables de Entrada", true);
        ...
        JPanel panelPresupuesto = new JPanel(new FlowLayout(FlowLayout.LEFT, 10, 5));
        panelPresupuesto.setBackground(new Color(255, 255, 200));
        panelPresupuesto.setBorder(BorderFactory.createTitledBorder("Presupuesto Total"));

        JLabel lblPresupuesto = new JLabel("Presupuesto: $");
        lblPresupuesto.setFont(new Font("Calibri", Font.BOLD, 14));
        JTextField txtPresupuesto = new JTextField(String.valueOf((int) Config.PRESUPUESTO), 15);
        txtPresupuesto.setFont(new Font("Calibri", Font.PLAIN, 14));

        panelPresupuesto.add(lblPresupuesto);
        panelPresupuesto.add(txtPresupuesto);

        // Panel para inversiones
        JPanel panelInversiones = new JPanel(new GridLayout(Config.NUM_PROYECTOS + 1, 2, 10, 8));
        panelInversiones.setBackground(Color.WHITE);
        panelInversiones.setBorder(BorderFactory.createTitledBorder("Inversión Inicial por Proyecto"));

        JLabel lblHeader1 = new JLabel("Proyecto");
        lblHeader1.setFont(new Font("Calibri", Font.BOLD, 13));
        JLabel lblHeader2 = new JLabel("Inversión Inicial ($)");
        lblHeader2.setFont(new Font("Calibri", Font.BOLD, 13));
        panelInversiones.add(lblHeader1);
        panelInversiones.add(lblHeader2);

        JTextField[] txtInversiones = new JTextField[Config.NUM_PROYECTOS];
        for (int i = 0; i < Config.NUM_PROYECTOS; i++) {
            JLabel lblProyecto = new JLabel("Proyecto " + (i + 1));
            lblProyecto.setFont(new Font("Calibri", Font.PLAIN, 12));

            txtInversiones[i] = new JTextField(String.valueOf((int) Config.INVERSION[i]), 12);
            txtInversiones[i].setFont(new Font("Calibri", Font.PLAIN, 12));

            panelInversiones.add(lblProyecto);
            panelInversiones.add(txtInversiones[i]);
        }
        ...
        btnGuardar.setBackground(new Color(68, 114, 196));
        ...
        btnGuardar.addActionListener(e -> {
            try {
                ...
                double nuevoPresupuesto = Double.parseDouble(txtPresupuesto.getText().replace(",", ""));
                if (nuevoPresupuesto <= 0) {
                    JOptionPane.showMessageDialog(dialogo, "El presupuesto debe ser mayor a 0", "Error de validación", JOptionPane.ERROR_MESSAGE);
                    return;
                }

                // Validar y guardar inversiones
                double[] nuevasInversiones = new double[Config.NUM_PROYECTOS];
                for (int i = 0; i < Config.NUM_PROYECTOS; i++) {
                    nuevasInversiones[i] = Double.parseDouble(txtInversiones[i].getText().replace(",", ""));
                    if (nuevasInversiones[i] < 0) {
                        JOptionPane.showMessageDialog(dialogo, "La inversión del Proyecto " + (i + 1) + " no puede ser negativa", "Error de validación", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                }
                ...
                Config.INVERSION = nuevasInversiones;

                // Actualizar interfaz
                llenarTablaInicial();
                actualizarLabel(lblTotales[0], "Presupuesto", Config.PRESUPUESTO);
                actualizarLabel(lblTotales[1], "Inversión", 0);
                actualizarLabel(lblTotales[2], "Sobrante", 0);
                actualizarLabel(lblTotales[3], "Ganancia total", 0);

                JOptionPane.showMessageDialog(dialogo, "Variables actualizadas correctamente.\nPresupuesto: " + FMT_MONEY.format(Config.PRESUPUESTO), "Éxito", JOptionPane.INFORMATION_MESSAGE);
                dialogo.dispose();

            } catch (NumberFormatException ex) {
                JOptionPane.showMessageDialog(dialogo, "Por favor, ingrese solo números válidos", "Error de formato", JOptionPane.ERROR_MESSAGE);
            }
        });
        ...
        JScrollPane scrollInversiones = new JScrollPane(panelInversiones);
        scrollInversiones.setPreferredSize(new Dimension(400, 300));

        panelPrincipal.add(panelPresupuesto, BorderLayout.NORTH);
        panelPrincipal.add(scrollInversiones, BorderLayout.CENTER);
    ...
    private void ejecutarOptimizacion() {
        ...
        btnGraficas.setEnabled(false);
        btnEditarVariables.setEnabled(false);
        ...
        new SwingWorker<Void, int[]>() {
            ...
            protected void done() {
                ...
                btnEditarVariables.setEnabled(true);
    ...
    private double calcularInversion(int[] decision) {
        return IntStream.range(0, Config.NUM_PROYECTOS).mapToDouble(i -> decision[i] * Config.INVERSION[i]).sum();
    }
    ...
    private void mostrarVentanaResultados() {
        ...
        main.add(new JLabel("Optimización terminada. Todas las variables enumeradas."), BorderLayout.NORTH);

        JPanel tablas = new JPanel(new GridLayout(3, 1, 5, 5));
        tablas.setBackground(Color.WHITE);
        tablas.add(crearSeccionResultado("Objetivos", "Valor", new String[]{"Maximizar Media Total profit"}, new String[]{FMT_MONEY.format(mejorGanancia)}));
        tablas.add(crearSeccionResultado("Restricciones", "Lado izq <= Lado der", new String[]{"Inversión <= Presupuesto"}, new String[]{FMT_MONEY.format(calcularInversion(mejorDecision)) + " <= " + FMT_MONEY.format(Config.PRESUPUESTO)}));
    ...
    private void mostrarHistograma() {
        ...
        JFreeChart chart = ChartFactory.createHistogram("Total profit", "Dollars", "Frecuencia", dataset, PlotOrientation.VERTICAL, false, true, false);
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> new OptimizadorSeleccionProyectosManual().setVisible(true));
    }
}